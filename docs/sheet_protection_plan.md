...
| Case_Master | 案件 ID/名前 (A,B 列)、メモ (C 列)、案件選択 `H1`（リスト: `CaseIds`） | `施策数`/`平均進捗` (D,E 列)、ドリルダウン領域 `CaseDrilldownArea` (G3:N104) | フィルター・並べ替えは許可。 |
...
## 案件ドリルダウンの参照範囲と名前定義
* 施策一覧は `MeasureList`（`Measure_Master!$A$2:$H$104`）として名前定義し、`Case_Master!G3` の `FILTER` スピルで参照する。絞り込み条件は親案件 ID（2 列目）で、範囲全体を `CaseDrilldownArea`（`Case_Master!$G$3:$N$104`）として保護対象にまとめる。
* 案件選択のプルダウンは `CaseIds`（`Case_Master!$A$2:$A$100`）を共通のデータ検証ソースとして `Case_Master!H1` と `Measure_Master!B2:B104` に適用する。選択セル `H1` はロック解除し、スピル先は保護したままにする。
