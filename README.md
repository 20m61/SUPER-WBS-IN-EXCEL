...
* **実進捗:** G 列。各 WBS シートのヘッダーセル（`J2`）を参照し、参照切れ時は「未リンク」を返す。`=IFERROR(INDIRECT("'" & F行 & "'!J2"), "未リンク")`
...
* **INDIRECT の参照切れ:** `Measure_Master!G列` は `IFERROR` で包み、対象 WBS シートが存在しない場合は「未リンク」と表示する。`Kanban_View` の `FILTER` 連鎖も `IFERROR` でガードし、選択したシートが見つからないときは「選択したWBSシートが見つかりません」と明示して #REF! を防ぐ。
* **データ検証外入力:** 案件 ID（`Case_Master!H1`、`Measure_Master!B列`）、WBS シート名セレクター（`Kanban_View!B2`）はリスト型データ検証に `stop` スタイルのエラーアラートを有効化し、「リストから選択してください」というメッセージで再入力を促す。プルダウン外の値はエラーで受け付けない。
* **警告の見せ方:** 参照切れで「未リンク」や警告メッセージが出た行は、必要に応じて条件付き書式で背景色を付ける運用を想定し、担当者にリカバリーを促す。
...
## 13. シート保護とパスワード運用
シートごとの編集可能列と保護対象、`.xlsm` 出力時の保護適用手順、VBA による解除・再適用ユーティリティ、パスワード共有・変更の手順を [docs/sheet_protection_plan.md](docs/sheet_protection_plan.md) にまとめた。ブック生成や運用前に必ず参照すること。
